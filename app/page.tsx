          ...
          >
            ...
            <button 
              onClick={async () => {
                ...
                if (!phone) return;
                const concern = prompt('What is your health concern? (optional)');
                
                try {
                  const response = await fetch('/api/booking/consultation', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name, phone, concern }),
                  });
                  const result = await response.json();
                  if (result.success) {
                    alert(`✅ Consultation Booked!\n\nConsultation ID: ${result.data.consultationId}\nDoctor: ${result.data.doctor}\nStatus: ${result.data.status}\n\nWe'll send you the meeting link via SMS/Email!`);
                  }
                } catch (error) {
                  alert('Failed to book consultation. Please try again.');
                }
              }}
              className="bg-white text-blue-600 px-8 py-4 rounded-2xl font-semibold hover:shadow-2xl hover:scale-105 transition-all"
            >
